...
"""
Code Developers of Dragongly can use this component to export Dragongly user objects and source code that they create to the Github folder on their computer.
...
-
Provided by Dragongly 0.0.02

    Args:
        _components: Any output from a new Dragongly component that you wish to export. Right now, only one component can be connected at a time but you can input a "*" (without quotation marsk) to search all changed Dragongly components on a grasshopper canvas.
        _targetFolder: A file path on your system which you would like to export the user object and source code to.  For most code developers, this file path will lead to their Github folder for Dragongly (or Honeybee), which is usually installed in "My Documents" by default. Exported source code will be saved at .\src and exported userObjects will be saved at .\userObjects in this _targetFolder.
        _export: Set to "True" to export Dragongly components to the _targerFolder.
    Returns:
        readMe!: ...
"""

ghenv.Component.Name = "DF Export Dragongly"
ghenv.Component.NickName = 'ExportDragongly'
ghenv.Component.Message = 'VER 0.0.66\nJUN_12_2018'
ghenv.Component.Category = "Dragongly"
...
def exportToUserObject(component, targetFolder, lb_preparation):
    ...
    def isNewerVersion(currentUO, component):
        ...
        for lineCount, line in enumerate(componentCode):
            ...
            if line.strip().startswith("ghenv.Component.Message"):
                ...
                # in case the file doesn't have an standard Dragongly message let it be updated
    ...
    # copy into Dragonfly
    filePathInit = os.path.join('\\'.join(filePath.split('\\')[:-2]), component.Name + ".ghuser")
            ...
            for lineCount, line in enumerate(pyFile):
                ...
                if line.strip().startswith("ghenv.Component.Message"):
                    ...
                    try:
                        ...
                        # in case the file doesn't have an standard Dragongly message
                        return True
